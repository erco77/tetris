    ...
    int x,y;            /* x: 1-80, y:1-24 */
{
    if (term)
    {
        if (term[0]=='w')          /* WYSE TERMINALS */
        {
            /* WYSE CURSOR POSITION */
            printf("%c%c%c%c",0x1b, 0x3d, 0x20+y-1, 0x20+x-1);
            return;
        }
    } 
 
    /* CURSOR POSITION FOR VT100/IRIS-ANSI/IBMPC ANSI TERMINALS */
    printf("\33[%d;%dH",y,x);
}
    ...
    int *x, *y, *rotate, *yforce;
{
    ...
    while (c=ReadKey())
    {
        switch(c)
        {
            ...
            /* TESTING ONLY */
            case   TEST: Gtest ^= 1; break;
            /* Gshape = (Gshape + 1) % MAXSHAPES; break; */
...
void HandleCompletedRows()
{
    ...
    if (trows)
    {
        ...
        Redraw(GSCREEN);
        Grows += trows;
        /* UpdateScore(0); Updated elsewhere */
    }
...
int HandleShape(int*,int*,int*,int*);   /*PROTOTYPE*/
int HandleShape(x,y,rotate,yforce)
    int *x, *y, *rotate, *yforce;
{
    ...
    while (1)
    {
        switch(DrawShape(Gx + *x, Gy + *y + *yforce, (Grotate + *rotate) % 4,
                         OLAPCHECK))
        {
